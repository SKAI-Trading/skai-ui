...
Rows (`7710:92634` and siblings): `px 16 / py 2`, Mulish Regular 12/16 at
`-0.48px`. Price w=70 left (ask `#FF574A`, bid `#17F9B4`); Size w=55 right,
white; Total w=70 right, white.
...
### What blocks the ladder

All of the above lives in `modules/skai-ui/src/components/trading/order-book.tsx`
and `@skai/ui` resolves to the built `dist`, so landing it needs a skai-ui change
plus a dist rebuild. The measurement is no longer the blocker; the build is.
