 ...
 *   node figma-catalog/coverage.mjs --markdown # print COVERAGE.md to stdout, write nothing
 *
 * ---------------------------------------------------------------------------
...
/*
  `--markdown` — the exact COVERAGE.md text on stdout, and nothing written.
  `pipeline.mjs --self-test` runs it twice under two different clocks and
  requires the same bytes, which is what keeps a run date out of this file.
*/
const MARKDOWN = process.argv.includes("--markdown");
const CHECK_ONLY = process.argv.includes("--check") || HISTOGRAM || FRAMES || MARKDOWN;
...
const P = (s = "") => md.push(s);
/*
  ★ NO RUN CLOCK IN THIS FILE. Every date below is read off an input.

  The title used to say `measured <today>`. `pipeline.mjs --check` compares each
  derived file with its timestamp lines removed, and a bare date is not a
  timestamp, so COVERAGE.md came out stale on every new calendar day with no
  input changed — measured 2026-09-24, it was the only stale file. A check that
  always fails cannot show the day a file really is stale. The run's own instant
  still exists, as `generated` in coverage.json (emit-parity-json.mjs publishes
  it), and the harvest's age in days is printed by pipeline.mjs on every run.
*/
const pageStamps = manifest.pages.map((p) => p.harvestedAt).filter(Boolean).sort();

P(`# Catalog coverage — live half harvested ${manifest.harvestedAt}`);
P();
...
P(
  pageStamps.length
    ? `Pages last read or hash-checked in Figma between ${pageStamps[0]} and ${pageStamps[pageStamps.length - 1]} (per-page \`harvestedAt\`, live/_pages.json).`
    : `No page in live/_pages.json records its own \`harvestedAt\`.`,
);
P(`This file carries no run date, so it is byte-identical whenever its inputs are. The run's own instant is \`generated\` in coverage.json; \`pipeline.mjs\` prints how many days old the harvest is.`);
P();
...
if (FRAMES) {
  ...
  // Exactly the bytes the default mode writes to COVERAGE.md.
