#!/usr/bin/env node
// figma:tokens: the Figma variables and styles the tracked frames use, held in figma/tokens/.
//
...
import { exportScript, provenanceScript, SCRIPT_CAP } from "./lib/tokens-plugin.mjs";
import {
  FILES,
  LIBRARY_FILE,
  paths,
...
export async function main(argv, root = HERE, catalogDir = path.join(HERE, "..", "figma-catalog")) {
  ...
  if (cmd === "status") {
    ...
    console.log(`tokens: ${st.vars.size} variables, ${st.text.size} text styles, ${st.effect.size} effect styles, ${st.paint.size} paint styles`);
  ...
  if (cmd === "get") {
    ...
    for (const t of hits.slice(0, 40)) {
      const e = t.entry;
      console.log(`${t.name}  [${t.kind}${t.kind === "variable" ? ` ${t.type}, collection ${t.collection}` : ""}]  key ${t.key}${e.remote ? " (library)" : " (local)"}`);
      if (t.kind === "variable") {
  ...
  if (cmd === "list") {
    ...
    for (const t of all) {
      ...
      else v = JSON.stringify(e.effects || e.paints);
  ...
  console.error("usage: node figma/tokens.mjs status | export-script | export-ingest | provenance-script | provenance-ingest | ledger-failed | get | list | diff | --self-test");
  return 2;
}
