    ...
    return (
      <div className="space-y-1">
        ...
        {description && !hasError && (
          <p id={effectiveDescriptionId} className="text-xs text-muted-foreground">
            {description}
          </p>
        )}
        {hasError && (
          <p id={effectiveErrorId} className="text-xs text-destructive" role="alert">
            {error}
          </p>
        )}
      </div>
...
const SkaiInput = React.forwardRef<HTMLInputElement, SkaiInputProps>(
  (
    ...
    const currentState = error ? "error" : state || (isFocused ? "focus" : "normal");
      ...
      >
        ...
        {(label || helperAction) && (
          <div className="flex items-center justify-between">
            ...
            {helperAction && (
              <div className="text-sm text-[#17F9B4]">{helperAction}</div>
            )}
          </div>
        )}
        ...
        <input
          ref={ref}
          className={cn(
            "w-full bg-transparent font-['Manrope'] tracking-[-0.04em] outline-none",
        ...
        {(secondaryValue || error) && (
          <div
            className={cn(
              "font-['Manrope'] text-xs tracking-[-0.04em]",
