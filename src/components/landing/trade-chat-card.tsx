...
const SUGGESTIONS = [
  ...
  { label: "Buy skaiUSD", icon: "token" as const, message: "How do I buy skaiUSD on Base?" },
  ...
  return (
    <div className="max-w-[92%] rounded-xl rounded-bl-sm bg-[#001615]/50 border border-[#95A09F]/10 px-[12px] py-[10px] font-['Manrope',sans-serif]">
      ...
      <p className="text-[13px] leading-[18px] text-[#E0E0E0] mb-[8px]">
        {hasClaimedDeposit
          ? "Deposit skaiUSD to fund your wallet and start trading with AI."
          : "Welcome! Deposit skaiUSD to earn 1 SKAI Point per skaiUSD and start trading with AI."}
      </p>
      ...
      {!hasClaimedDeposit && (
        <div className="flex items-center gap-[6px] mb-[8px] px-[8px] py-[6px] rounded-md bg-[#0D3D3A]/60 border border-[#2DEDAD]/15">
          ...
          <span className="text-[#8B9E9D] text-[11px] leading-[14px]">
            1 SKAI Point per skaiUSD on your first deposit (one-time reward)
          </span>
        </div>
      )}
  ...
  (
    ...
    useEffect(() => {
      ...
      initial.push({
        ...
        role: "assistant",
        content: username
          ? `Hey ${username}! I'm your AI trading assistant. Ask me anything — swap tokens, buy skaiUSD or ETH, analyze charts, check your portfolio, or bridge assets.`
          : "Hey! I'm your AI trading assistant. Ask me anything — market signals, trade ideas, chart analysis, swap tokens, or buy skaiUSD.",
        timestamp: Date.now(),
      });
        ...
        if (onSendMessage) {
          ...
          setMessages((prev) => [
            ...prev,
            {
              id: nextId(),
              role: "assistant" as const,
              content: "I'm not connected to the AI backend yet. Try using the quick actions below or deposit skaiUSD to get started.",
              timestamp: Date.now(),
            },
          ]);
        }
